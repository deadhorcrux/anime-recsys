...
class MyALSModel:
    def __init__(self, n_factors: int, iterations: int, top_k: int, mapper: Dict) -> None:
        self._top_k = top_k
        self._mapper = mapper
    ...
    def fit(self, events: pd.DataFrame, users: pd.DataFrame, items: pd.DataFrame) -> "MyALSModel":
    ...
    def predict(self, user_ids: NDArray[np.int32]) -> pl.DataFrame:
        ...
        recs = pl.concat([hot_users_recommendations, cold_users_recommendations], how="diagonal")
        recs = recs.to_pandas()
        recs['item_id'] = recs['item_id'].map(self._mapper.get)
        return recs
